...
public abstract class Entity
{
    ...
    public void moveEntity(double par1, double par3, double par5)
    {
    	...
    	if(posX + par1 < negbounds){
    		par1 = negbounds - posX;
    	}
    	if(posZ + par5 > bounds){
    		par5 = bounds - posZ;
    	}
    	if(posZ + par5 < negbounds){
    		par5 = negbounds - posZ;
    	}
    ...
    public boolean isInRangeSimple(Vec3D par1Vec3D, int dist){
        int d = (int)(posX - par1Vec3D.xCoord);
        int d1 = (int)(posY - par1Vec3D.yCoord);
        int d2 = (int)(posZ - par1Vec3D.zCoord);
        if(d > dist || d < -dist)
        	return false;
